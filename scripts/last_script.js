...
function getResult(multiplier) {
    ...
    if (info.run) {
        ...
        if (multiplier < info.currentPayout) {
            ...
            for (let index = 1.5; index <= 5; index += 0.5) {
            ...
            if (info.losses <= 0) {
                ...
                if (info.afterBalance < info.beforeBalance) {
                    info.beforeBalance = info.currentBalance;
    ...
    info.currentBet = getNextBet(info.losses, info.currentPayout);
    if (info.run) {
